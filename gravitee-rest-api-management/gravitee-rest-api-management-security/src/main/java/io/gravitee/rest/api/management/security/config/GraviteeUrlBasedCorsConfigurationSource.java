...
public class GraviteeUrlBasedCorsConfigurationSource extends UrlBasedCorsConfigurationSource {
    ...
    public CorsConfiguration getCorsConfiguration(HttpServletRequest request) {
        ...
        if (path.indexOf("/organizations/") > -1) {
            int orgIdStartIndex = path.indexOf("/organizations/") + "/organizations/".length();
            String organizationId = path.substring(orgIdStartIndex, path.indexOf('/', orgIdStartIndex));
